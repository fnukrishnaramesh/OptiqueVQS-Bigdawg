...
//BLOCKED: Annoying
//move attribute selected for output to the top of list
function moveUp(attr, cls) {
	/*	if (!attr.hasClass('output') && !attr.hasClass('constraint')) {
	 attr.siblings(':first').hide().before(attr).fadeIn("slow");
	 $("#ul_" + facet_id).listview('refresh');
	 attr.addClass('movedUp');
	 }
	 attr.addClass(cls);*/
}

// BLOCKED: Annoying
//move attribute unselected for output to the end of list
function moveDown(attr, cls) {
	/*var test = (cls == 'output' ? 'constraint' : 'output')
	 attr.removeClass(cls);
	 if (!attr.hasClass(test)) {
	 attr.removeClass('movedUp');
	 if (attr.siblings('.movedUp:last').length > 0) {
	 attr.siblings('.movedUp:last').hide().after(attr).fadeIn("slow");
	 $("#ul_" + facet_id).listview('refresh');
	 }
	 }*/
}
