		...
		if (cmd.type == 'nodeAdded') {
			...
			for (var i = 0; i < cmd.content.dt.length; i++) {
				...
				if (cmd.content.dt[i].seq !== undefined)
					tree.updateSequence(cmd.content.dt[i].seq, 'noStack');
...
function myTree() {
	...
	this.addSame = function(node1, node2) {
		...
		// TODO: integration with history undo/redo
		// TODO: color management
		//console.log(JSON.stringify(tree.getQueryGraph()));
	}
	...
	this.updateStreamOp = function(dt, stk) {
		...
		//if sequence applied over aggregate
		//if (node.data.sequence.vr == node.data.aggregate.vra) {
		//	node.data.sequence.vro = node.data.aggregate.vr;
		//}
		...
		//update sparql view
		//updateSparqlView();
		//you need to combine these to informChange
		//tree.deliverEvent('', 'executeQuery');
	...
	this.updateAggregate = function(dt, stk) {
		...
		if (!jQuery.isEmptyObject(node.data.sequence)) {
			...
			// NOT REQUIRED
			/*if (jQuery.isEmptyObject(node.data.aggregate) && node.data.aggregate.vr == prev.vr) {
			console.log("1");
	...
	this.addNode = function(source, data, stk, isActive) {
		...
		//st.addNodeInPath(target.id);
		e.data = data.prop;

		//restore seq data
		/*if (data.seq !== undefined)
		 hnode.data.sequence = data.seq;
		 //restore agg data
		 if (data.agg !== undefined)
		 hnode.data.aggregate = data.agg;*/
	...
	this.toSparqlObj = function() {
				...
				if ( typeof node.data.constraint !== 'undefined') {
					$.each(node.data.constraint, function(index, result) {
						...
						if (result['id'] != rdf + 'type') {
							...
							if (isStream() > 0) {
								tree.objPush(SparqlObj.slcdf, '?' + node.id + ' ' + tree.assignNsId(prfxArr, result['ns']) + ':' + result['name'] + ' "' + result['constr'] + '"^^xsd:' + result['type'] + '.');
								tree.objPush(SparqlObj.slcdfs, '?' + node.data.hId + ' ' + tree.assignNsId(prfxArr, result['ns']) + ':' + result['name'] + ' "' + result['constr'] + '"^^xsd:' + result['type'] + '.');
							} else {
								tree.objPush(SparqlObj.slcdf, '?' + node.id + ' ' + tree.assignNsId(prfxArr, result['ns']) + ':' + result['name'] + ' ?' + result['aId'] + '.');
								tree.objPush(SparqlObj.slcdfs, '?' + node.data.hId + ' ' + tree.assignNsId(prfxArr, result['ns']) + ':' + result['name'] + ' ?' + result['hId'] + '.');
							}
							...
							if (result['constrType'] == 'eq' && isStream() <= 0) {
								...
								if (result['type'] == 'dateTime') {
									...
									tree.objPush(SparqlObj.flt, 'FILTER(?' + result['aId'] + ' = "' + date1.toISOString() + '"^^xsd:dateTime)' + ext + '.');
									tree.objPush(SparqlObj.flts, 'FILTER(?' + result['hId'] + ' = "' + date1.toISOString() + '"^^xsd:dateTime)' + ext + '.');
								} else {
								...
								if (result['type'] == 'dateTime') {
									...
									tree.objPush(SparqlObj.flt, 'FILTER(?' + result['aId'] + ' >= "' + date1.toISOString() + '"^^xsd:dateTime && ?' + result['aId'] + ' <= "' + date2.toISOString() + '"^^xsd:dateTime)' + ext + '.');
									tree.objPush(SparqlObj.flts, 'FILTER(?' + result['hId'] + ' >= "' + date1.toISOString() + '"^^xsd:dateTime && ?' + result['hId'] + ' <= "' + date2.toISOString() + '"^^xsd:dateTime)' + ext + '.');
								} else {
		...
		if (!jQuery.isEmptyObject(root.data.sequence)) {
			if (root.data.sequence.op == "sortdown") {
				...
				SparqlObj.seqs = "ORDER BY DESC(?" + (root.data.aggregate.vra == root.data.sequence.vr ? root.data.aggregate.vrha : tree.gethId(root.data.sequence.vr)) + ")";
			}
			if (root.data.sequence.op == "sortup") {
				SparqlObj.seq = "ORDER BY ASC(?" + root.data.sequence.vr + ")";
				SparqlObj.seqs = "ORDER BY ASC(?" + (root.data.aggregate.vra == root.data.sequence.vr ? root.data.aggregate.vrha : tree.gethId(root.data.sequence.vr)) + ")";
			}
		}
		...
		if (!jQuery.isEmptyObject(root.data.aggregate)) {
			...
			SparqlObj.slcts[i] = "(" + root.data.aggregate.op.toUpperCase() + "(?" + tree.gethId(root.data.aggregate.vr) + ") AS ?" + root.data.aggregate.vrha + ")";
	...
	this.getSparql = function(SparqlObj) {
		...
		//delete later
		//query = query.replace(/npdv:\^/g, '^npdv:');
		query = tree.invCorr(query);

		// TODO: check later
		/*query = '#@ext_activeNode' + ' ' + activeNode + '\n\n' + query;

		if (SparqlObj.streamp != '') {
			var p = ""
			for (var key in SparqlObj.streamp) {
				p += '#@stream_property ' + SparqlObj.streamp[key] + '\n';
			}

			if (p != "")
				query = p + query;
		}
		if (SparqlObj.streamv != '') {
			var v = "";

			for (var key in SparqlObj.streamv) {
				v += '#@stream_' + key + ' ' + '"' + SparqlObj.streamv[key] + '"';
				if (key == "slide" || key == "window")
					v += '^^xsd:duration' + '\n'
	...
	this.getStarql = function(sparqlObj) {
		...
		var streamnames = decodeURIComponent(getURLParameter($(parent.location).attr('href'), "stream")).split(",");
		var staticAboxes = decodeURIComponent(getURLParameter($(parent.location).attr('href'), "abox")).split(",");
		...
		var streamslide = '"PT' + sparqlObj.streamv['slide'] + sparqlObj.streamv['slider'] + '"';
		var streamwindow = '"PT' + sparqlObj.streamv['window'] + sparqlObj.streamv['windowr'] + '"';
		var streamstart = sparqlObj.streamv['start'];
		var streamend = sparqlObj.streamv['end'];

		for (var i = 0; i < sparqlObj.prfx.length; i++)
			query += sparqlObj.prfx[i] + '\n';
		query += 'CREATE PULSE pulseA WITH FREQUENCY = ' + streamslide + '^^xsd:duration\n';
		...
		// Additionally, select everything in the query that is not already a streaming predicate.
		remove_duplicates:
		for (var i = 0; i < slct.length; i++) {
			for (var j = 0; j < streampred.length; j++) {
				for (var k = 0; k < slcdf.length; k++) {
					var match1 = slcdf[k].indexOf(streampred[j]);
					var match2 = slcdf[k].indexOf(slct[i]);
					if (match1 > -1 && match2 > -1 && match2 > match1) {
						// Don't select thing if it's an obj whose pred is a streaming predicate
						continue remove_duplicates;
					}
				}
			}
			query += " " + slct[i];
		...
		for(var i = 0; i < streamnames.length; i++) {
			query += sep;
			sep = ', ';
			query += streamnames[i] + ' [NOW - ' + streamwindow + '^^xsd:duration, NOW]->' + streamslide + '^^xsd:duration';
		}
		if (staticAboxes.length > 0){
			sep = ', \r\n'
			query += sep + 'STATIC ABOX ';
			sep = ' ';
			for(var i = 0; i < staticAboxes.length; i++) {
				query += sep;
				sep = ', ';
				query += staticAboxes[i];
			}
		}
		query += '\nUSING PULSE pulseA';
		query += '\nWHERE {\n';
		...
		// TODO: figure out which of these clauses make sense for a
		// streaming query / are legal in STARQL
		/*
		 if (grpby != '')
		 query += " " + grpby;
		 if (seq != '')
	...
	this.getSparqlView = function() {
		...
		// TODO: check later
		/*query = '#@ext_activeNode' + ' ' + activeNode + '<br><br>' + query;

		if (SparqlObj.streamp != '') {
			var p = ""
			for (var key in SparqlObj.streamp) {
				p += '#@stream_property ' + SparqlObj.streamp[key] + '<br>';
			}
	...
	this.deliverEvent = function(nodeId, type) {
		...
		if (type == 'nodeAdded' || type == 'nodeSelected') {
			...
			//} else {
			//Channel.message.content.slide = '';
			//Channel.message.content.window = '';
			//Channel.message.content.start = '';
			//Channel.message.content.end = '';
			//}
